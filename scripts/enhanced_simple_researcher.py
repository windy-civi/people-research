#!/usr/bin/env python3
"""
Enhanced Simple Researcher Script

This script directly iterates through OpenStates people data, checks if we already have
research for each person, and if not, performs the research using the enhanced researcher
and saves directly to the repo.
"""
...
class EnhancedSimpleResearcher:
    def __init__(self, openstates_path: str, research_repo_path: str):
        """Initialize the enhanced simple researcher."""
    ...
    def run(self):
        ...
        logger.info(f"Starting enhanced research for locale: {self.locale or 'all'}")
        ...
        logger.info(f"Enhanced research completed: {processed} people processed")
    ...
    def _process_person_file(self, yaml_file: str) -> Optional[Dict[str, Any]]:
        ...
        try:
            ...
            state_index = path_parts.index('data') + 1
            state = path_parts[state_index] if state_index < len(path_parts) else 'unknown'
            ...
            if os.path.exists(output_path):
                logger.debug(f"Research already exists for {person_data.get('name', 'Unknown')}")
                return None
    ...
    def _has_active_legislative_role(self, person_data: Dict[str, Any]) -> bool:
        ...
        for role in roles:
            ...
            if role_type in ['upper', 'lower', 'legislature']:
                # Check if role is current (no end date or end date is in the future)
    ...
    def _research_person(self, person: Dict[str, Any]):
        """Research a single person using the enhanced researcher."""
        ...
        # Import the enhanced researcher here to avoid circular imports
        from enhanced_legislator_researcher import EnhancedLegislatorResearcher
        
        # Create enhanced researcher instance
        ...
        # Perform enhanced research with web search capabilities
        ...
        logger.info(f"Saved enhanced research for {person['name']} to {person['output_path']}")

def main():
    """Main function."""
    if len(sys.argv) != 3:
        print("Usage: python enhanced_simple_researcher.py <openstates-people-repo-path> <research-git-repo-path>")
        print("Example: python enhanced_simple_researcher.py ./openstates-people .")
        sys.exit(1)
    ...
    try:
        researcher = EnhancedSimpleResearcher(openstates_path, research_repo_path)
        researcher.run()
    except Exception as e:
